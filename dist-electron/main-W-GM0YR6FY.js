import require$$1$3, { dialog, ipcMain, app, BrowserWindow } from "electron";
...
var debug$3 = noop;
if (util$2.debuglog) debug$3 = util$2.debuglog("gfs4");
...
function patch$2(fs2) {
  ...
  if (fs$copyFile) fs2.copyFile = copyFile2;
  ...
  function readdir(path2, options, cb) {
    ...
    var go$readdir = noReaddirOptionVersions.test(process.version)
      ? function go$readdir2(path22, options2, cb2, startTime) {
          return fs$readdir(
            path22,
            fs$readdirCallback(path22, options2, cb2, startTime),
          );
        }
      : function go$readdir2(path22, options2, cb2, startTime) {
          return fs$readdir(
...
utils$1.checkPath = function checkPath(pth) {
  if (process.platform === "win32") {
    const pathHasInvalidWinCharacters = /[<>:"|?*]/.test(
      pth.replace(path$m.parse(pth).root, ""),
    );
...
function getStats$2(src2, dest, opts) {
  const statFunc = opts.dereference
    ? (file2) => fs$d.stat(file2, { bigint: true })
    : (file2) => fs$d.lstat(file2, { bigint: true });
  return Promise.all([
    statFunc(src2),
...
function getStatsSync(src2, dest, opts) {
  let destStat;
  const statFunc = opts.dereference
    ? (file2) => fs$d.statSync(file2, { bigint: true })
    : (file2) => fs$d.lstatSync(file2, { bigint: true });
  ...
  util$1.callbackify(getStats$2)(src2, dest, opts, (err, stats) => {
    ...
    if (destStat) {
      ...
      if (!srcStat.isDirectory() && destStat.isDirectory()) {
        return cb(
          new Error(
            `Cannot overwrite directory '${dest}' with non-directory '${src2}'.`,
          ),
        );
      }
    }
...
function checkParentPaths(src2, srcStat, dest, funcName, cb) {
  ...
  if (destParent === srcParent || destParent === path$l.parse(destParent).root)
    return cb();
...
function handleFilter(onInclude, destStat, src2, dest, opts, cb) {
  Promise.resolve(opts.filter(src2, dest)).then(
    (include) => {
    ...
    if (!destStat) {
      ...
      fs$c.readlink(dest, (err2, resolvedDest) => {
        ...
        if (
          ...
          stat$3.isSrcSubdir(resolvedDest, resolvedSrc)
        ) {
          return cb(
            new Error(
...
function onLink(destStat, src2, dest, opts) {
  ...
  if (!destStat) {
    ...
    if (
      ...
      stat$2.isSrcSubdir(resolvedDest, resolvedSrc)
    ) {
      throw new Error(
        `Cannot overwrite '${resolvedDest}' with '${resolvedSrc}'.`,
      );
...
const emptyDir = u$6(async function emptyDir2(dir) {
  ...
  return Promise.all(
    items.map((item) => remove$1.remove(path$h.join(dir, item))),
  );
});
...
function createSymlink$1(srcpath, dstpath, type2, callback) {
  ...
  fs$3.lstat(dstpath, (err, stats) => {
    if (!err && stats.isSymbolicLink()) {
      Promise.all([fs$3.stat(srcpath), fs$3.stat(dstpath)]).then(
        ([srcStat, dstStat]) => {
...
function stringify$4(
  ...
  if (str2 === void 0) {
    throw new TypeError(
      `Converting ${typeof obj} value to JSON is not supported`,
    );
  }
  return str2.replace(/\n/g, EOL) + EOF;
}
...
CancellationToken$1.CancellationError = CancellationToken$1.CancellationToken =
  void 0;
...
Object.defineProperty(ProgressCallbackTransform$1, "__esModule", {
  value: true,
});
    ...
    if (now >= this.nextUpdate && this.transferred !== this.total) {
      ...
      this.onProgress({
        ...
        bytesPerSecond: Math.round(
          this.transferred / ((now - this.start) / 1e3),
        ),
      });
      this.delta = 0;
    }
...
ProgressCallbackTransform$1.ProgressCallbackTransform =
  ProgressCallbackTransform;
Object.defineProperty(httpExecutor, "__esModule", { value: true });
httpExecutor.DigestTransform =
  httpExecutor.HttpExecutor =
  httpExecutor.HttpError =
    void 0;
...
const SENSITIVE_REDIRECT_HEADERS = /* @__PURE__ */ new Set([
  "authorization",
  "proxyauthorization",
  "privatetoken",
  "xapikey",
...
function parseJson(result) {
  return result.then((it) =>
    it == null || it.length === 0 ? null : JSON.parse(it),
  );
}
class HttpExecutor {
  constructor() {
  ...
  request(
    options,
    cancellationToken = new CancellationToken_1$1.CancellationToken(),
    data,
  ) {
  ...
  handleResponse(
    ...
    if (debug$2.enabled) {
      const { headers: _headers, auth: _auth, ...safeOptions } = options;
      debug$2(
        `Response: ${response.statusCode} ${response.statusMessage}, request options: ${safeStringifyJson(safeOptions)}`,
      );
    }
...
`,
    ...
    if (shouldRedirect && redirectUrl != null) {
      ...
      this.doApiRequest(
        HttpExecutor.prepareRedirectUrlOptions(redirectUrl, options),
        cancellationToken,
        requestProcessor,
        redirectCount,
      )
        ...
        if (response.statusCode != null && response.statusCode >= 400) {
          ...
          const isJson =
            ...
            (Array.isArray(contentType)
              ? contentType.find((it) => it.includes("json")) != null
  ...
  async downloadToBuffer(url, options) {
    return await options.cancellationToken.createPromise(
  ...
  doDownload(requestOptions, options, redirectCount) {
    const request = this.createRequest(requestOptions, (response) => {
      ...
      if (redirectUrl != null) {
        if (redirectCount < this.maxRedirects) {
          this.doDownload(
            HttpExecutor.prepareRedirectUrlOptions(redirectUrl, requestOptions),
            options,
            redirectCount++,
          );
        } else {
    ...
    this.addErrorAndTimeoutHandlers(
      request,
      options.callback,
  ...
  static prepareRedirectUrlOptions(redirectUrl, options) {
    const newOptions = configureRequestOptionsFromUrl(redirectUrl, {
      ...options,
    });
    ...
    if (HttpExecutor.isCrossOriginRedirect(originalUrl, parsedRedirectUrl)) {
      if (debug$2.enabled) {
        debug$2(
          `Cross-origin redirect (${originalUrl.host} → ${parsedRedirectUrl.host}): stripping sensitive headers`,
        );
      }
  ...
  static isCrossOriginRedirect(originalUrl, redirectUrl) {
    if (
      originalUrl.hostname.toLowerCase() !== redirectUrl.hostname.toLowerCase()
    ) {
      return true;
    }
    if (
      originalUrl.protocol === "http:" && // This can be replaced with `!originalUrl.port`, but for the sake of clarity.
      ["80", ""].includes(originalUrl.port) &&
      redirectUrl.protocol === "https:" && // This can be replaced with `!redirectUrl.port`, but for the sake of clarity.
    ...
    for (let attemptNumber = 0; ; attemptNumber++) {
      try {
        ...
        if (
          ...
          ((e instanceof HttpError && e.isServerError()) || e.code === "EPIPE")
        ) {
...
function checkSha2(sha2Header, sha2, callback) {
  if (sha2Header != null && sha2 != null && sha2Header !== sha2) {
    callback(
      new Error(
        `checksum mismatch: expected ${sha2} but got ${sha2Header} (X-Checksum-Sha2 header)`,
      ),
    );
    return false;
  }
...
function configurePipes(options, response) {
  if (
    !checkSha2(
      safeGetHeader(response, "X-Checksum-Sha2"),
      options.options.sha2,
      options.callback,
    )
  ) {
    return;
  ...
  if (options.options.onProgress != null) {
    ...
    if (contentLength != null) {
      streams.push(
        new ProgressCallbackTransform_1.ProgressCallbackTransform(
...
function isSensitiveFieldName(name) {
  ...
  return (
    SENSITIVE_FIELD_PATTERNS.some((p) => normalized.includes(p)) ||
    SENSITIVE_FIELD_SUFFIXES.some((s) => normalized.endsWith(s))
  );
}
...
function equals(firstValue, secondValue) {
  ...
  if (isFirstObject && isSecondObject) {
    ...
    return (
      ...
      keys1.every((key) => equals(firstValue[key], secondValue[key]))
    );
  }
...
function s3Url(options) {
  ...
  if (options.accelerate == true) {
    ...
    if (options.region == null) {
      throw new Error(
        `Bucket name "${options.bucket}" includes a dot, but S3 region is missing`,
      );
    }
...
function spacesUrl(options) {
  ...
  return appendPath(
    `https://${options.name}.${options.region}.digitaloceanspaces.com`,
    options.path,
  );
}
var retry$1 = {};
  ...
  try {
    ...
    if (
      (await Promise.resolve(
        (_a =
          shouldRetry === null || shouldRetry === void 0
            ? void 0
            : shouldRetry(error2)) !== null && _a !== void 0
          ? _a
          : true,
      )) &&
      retryCount > 0 &&
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, interval + backoff * attempt),
      );
...
function parseDn(seq2) {
  ...
  for (let i = 0; i <= seq2.length; i++) {
    ...
    if (ch === " " && !quoted) {
      ...
      if (
        nextNonSpace >= seq2.length ||
        seq2[nextNonSpace] === "," ||
        seq2[nextNonSpace] === ";" ||
        (key === null && seq2[nextNonSpace] === "=") ||
        (key !== null && seq2[nextNonSpace] === "+")
      ) {
        i = nextNonSpace - 1;
        continue;
  ...
  switch (encoding) {
    ...
    default:
      result =
        ...
        byte2hex[buffer[5]] +
        "-" +
        byte2hex[(buffer[6] & 15) | version] +
        byte2hex[buffer[7]] +
        "-" +
        byte2hex[(buffer[8] & 63) | 128] +
        byte2hex[buffer[9]] +
        "-" +
        byte2hex[buffer[10]] +
        byte2hex[buffer[11]] +
...
(function (exports) {
  (function (sax2) {
    ...
    function getDeclaredEncoding(body) {
      var match =
        body && body.match(/(?:^|\s)encoding\s*=\s*(['"])([^'"]+)\1/i);
          ...
          case S.ATTRIB_VALUE_ENTITY_U:
            ...
            if (c === ";") {
              ...
              if (
                ...
                !Object.values(sax2.XML_ENTITIES).includes(parsedEntity)
              ) {
...
function isNameEquals(element, name, ignoreCase) {
  ...
  return (
    elementName === name ||
    (ignoreCase === true &&
      elementName.length === name.length &&
...
function isValidKey(key) {
  ...
  return (
    ["string", "number", "symbol", "boolean"].includes(typeof key) ||
    key === null
  );
}
function asArray(v) {
...
function objectToArgs(obj) {
  const args = Object.entries(obj).reduce((args2, [name, value]) => {
    ...
    if (!SAFE_FLAG_NAME_RE.test(name)) {
      throw new Error(
        `objectToArgs: unsafe flag name rejected: ${JSON.stringify(name)}`,
      );
    }
    if (UNSAFE_VALUE_RE.test(value)) {
      throw new Error(
        `objectToArgs: value for --${name} contains a null byte or newline`,
      );
    }
...
(function (exports) {
  ...
  exports.CURRENT_APP_PACKAGE_FILE_NAME =
    exports.CURRENT_APP_INSTALLER_FILE_NAME =
    exports.objectToArgs =
    exports.deepAssign =
    exports.asArray =
    exports.mapToObject =
    exports.isValidKey =
    ...
    exports.parseDn =
    exports.retry =
    exports.githubTagPrefix =
    exports.githubUrl =
    exports.getS3LikeProviderBaseUrl =
    exports.ProgressCallbackTransform =
    exports.MemoLazy =
    exports.safeStringifyJson =
    exports.safeGetHeader =
    exports.parseJson =
    exports.isSensitiveFieldName =
    exports.HttpExecutor =
    exports.hashSensitiveValue =
    exports.HttpError =
    exports.DigestTransform =
    exports.createHttpError =
    exports.configureRequestUrl =
    exports.configureRequestOptionsFromUrl =
    exports.configureRequestOptions =
    exports.newError =
...
function formatError(exception2, compact) {
  ...
  where +=
    "(" + (exception2.mark.line + 1) + ":" + (exception2.mark.column + 1) + ")";
...
function makeSnippet$1(mark, options) {
  ...
  for (let i = 1; i <= options.linesBefore; i++) {
    ...
    result =
      common$4.repeat(" ", options.indent) +
  ...
  result +=
    ...
    "\n";
  result +=
    common$4.repeat("-", options.indent + lineNoLength + 3 + line.pos) + "^\n";
...
function isInteger(object) {
  return (
    ...
    object % 1 === 0 &&
    !common$3.isNegativeZero(object)
  );
}
...
function State$1(input, options) {
  ...
  this.maxTotalMergeKeys =
    typeof options["maxTotalMergeKeys"] === "number"
      ? options["maxTotalMergeKeys"]
      : 1e4;
...
(function (module, exports) {
  ...
  createToken(
    "MAINVERSION",
    `(${src2[t2.NUMERICIDENTIFIER]})\\.(${src2[t2.NUMERICIDENTIFIER]})\\.(${src2[t2.NUMERICIDENTIFIER]})`,
  );
  createToken(
    "MAINVERSIONLOOSE",
    `(${src2[t2.NUMERICIDENTIFIERLOOSE]})\\.(${src2[t2.NUMERICIDENTIFIERLOOSE]})\\.(${src2[t2.NUMERICIDENTIFIERLOOSE]})`,
  );
  createToken(
    "PRERELEASEIDENTIFIER",
    `(?:${src2[t2.NONNUMERICIDENTIFIER]}|${src2[t2.NUMERICIDENTIFIER]})`,
  );
  createToken(
    "PRERELEASEIDENTIFIERLOOSE",
    `(?:${src2[t2.NONNUMERICIDENTIFIER]}|${src2[t2.NUMERICIDENTIFIERLOOSE]})`,
  );
  createToken(
    "PRERELEASE",
    `(?:-(${src2[t2.PRERELEASEIDENTIFIER]}(?:\\.${src2[t2.PRERELEASEIDENTIFIER]})*))`,
  );
  createToken(
    "PRERELEASELOOSE",
    `(?:-?(${src2[t2.PRERELEASEIDENTIFIERLOOSE]}(?:\\.${src2[t2.PRERELEASEIDENTIFIERLOOSE]})*))`,
  );
  createToken("BUILDIDENTIFIER", `${LETTERDASHNUMBER}+`);
  createToken(
    "BUILD",
    `(?:\\+(${src2[t2.BUILDIDENTIFIER]}(?:\\.${src2[t2.BUILDIDENTIFIER]})*))`,
  );
  createToken(
    "FULLPLAIN",
    `v?${src2[t2.MAINVERSION]}${src2[t2.PRERELEASE]}?${src2[t2.BUILD]}?`,
  );
  createToken("FULL", `^${src2[t2.FULLPLAIN]}$`);
  createToken(
    "LOOSEPLAIN",
    `[v=\\s]*${src2[t2.MAINVERSIONLOOSE]}${src2[t2.PRERELEASELOOSE]}?${src2[t2.BUILD]}?`,
  );
  createToken("LOOSE", `^${src2[t2.LOOSEPLAIN]}$`);
  createToken("GTLT", "((?:<|>)?=?)");
  createToken(
    "XRANGEIDENTIFIERLOOSE",
    `${src2[t2.NUMERICIDENTIFIERLOOSE]}|x|X|\\*`,
  );
  createToken("XRANGEIDENTIFIER", `${src2[t2.NUMERICIDENTIFIER]}|x|X|\\*`);
  createToken(
    "XRANGEPLAIN",
    `[v=\\s]*(${src2[t2.XRANGEIDENTIFIER]})(?:\\.(${src2[t2.XRANGEIDENTIFIER]})(?:\\.(${src2[t2.XRANGEIDENTIFIER]})(?:${src2[t2.PRERELEASE]})?${src2[t2.BUILD]}?)?)?`,
  );
  createToken(
    "XRANGEPLAINLOOSE",
    `[v=\\s]*(${src2[t2.XRANGEIDENTIFIERLOOSE]})(?:\\.(${src2[t2.XRANGEIDENTIFIERLOOSE]})(?:\\.(${src2[t2.XRANGEIDENTIFIERLOOSE]})(?:${src2[t2.PRERELEASELOOSE]})?${src2[t2.BUILD]}?)?)?`,
  );
  createToken("XRANGE", `^${src2[t2.GTLT]}\\s*${src2[t2.XRANGEPLAIN]}$`);
  createToken(
    "XRANGELOOSE",
    `^${src2[t2.GTLT]}\\s*${src2[t2.XRANGEPLAINLOOSE]}$`,
  );
  createToken(
    "COERCEPLAIN",
    `${"(^|[^\\d])(\\d{1,"}${MAX_SAFE_COMPONENT_LENGTH2}})(?:\\.(\\d{1,${MAX_SAFE_COMPONENT_LENGTH2}}))?(?:\\.(\\d{1,${MAX_SAFE_COMPONENT_LENGTH2}}))?`,
  );
  createToken("COERCE", `${src2[t2.COERCEPLAIN]}(?:$|[^\\d])`);
  createToken(
    "COERCEFULL",
    src2[t2.COERCEPLAIN] +
      `(?:${src2[t2.PRERELEASE]})?(?:${src2[t2.BUILD]})?(?:$|[^\\d])`,
  );
  ...
  createToken(
    "TILDELOOSE",
    `^${src2[t2.LONETILDE]}${src2[t2.XRANGEPLAINLOOSE]}$`,
  );
  ...
  createToken(
    "CARETLOOSE",
    `^${src2[t2.LONECARET]}${src2[t2.XRANGEPLAINLOOSE]}$`,
  );
  createToken(
    "COMPARATORLOOSE",
    `^${src2[t2.GTLT]}\\s*(${src2[t2.LOOSEPLAIN]})$|^$`,
  );
  createToken("COMPARATOR", `^${src2[t2.GTLT]}\\s*(${src2[t2.FULLPLAIN]})$|^$`);
  createToken(
    "COMPARATORTRIM",
    `(\\s*)${src2[t2.GTLT]}\\s*(${src2[t2.LOOSEPLAIN]}|${src2[t2.XRANGEPLAIN]})`,
    true,
  );
  exports.comparatorTrimReplace = "$1$2$3";
  createToken(
    "HYPHENRANGE",
    `^\\s*(${src2[t2.XRANGEPLAIN]})\\s+-\\s+(${src2[t2.XRANGEPLAIN]})\\s*$`,
  );
  createToken(
    "HYPHENRANGELOOSE",
    `^\\s*(${src2[t2.XRANGEPLAINLOOSE]})\\s+-\\s+(${src2[t2.XRANGEPLAINLOOSE]})\\s*$`,
  );
  createToken("STAR", "(<|>)?=?\\s*\\*");
    ...
    if (release.startsWith("pre")) {
      ...
      if (identifier) {
        const match = `-${identifier}`.match(
          this.options.loose ? re$1[t$1.PRERELEASELOOSE] : re$1[t$1.PRERELEASE],
        );
  ...
  class Range2 {
    ...
    parseRange(range2) {
      ...
      const comparators = rangeList.map(
        (comp) => new Comparator2(comp, this.options),
      );
...
const gtr$1 = (version, range2, options) =>
  outside$2(version, range2, ">", options);
var gtr_1 = gtr$1;
const outside$1 = outside_1;
const ltr$1 = (version, range2, options) =>
  outside$1(version, range2, "<", options);
var ltr_1 = ltr$1;
  ...
  for (const c of dom) {
    ...
    if (gt2) {
      ...
      if (c.operator === ">" || c.operator === ">=") {
        ...
        gt2.operator === ">=" &&
        !satisfies$1(gt2.semver, String(c), options)
      ) {
        return false;
      }
    }
    if (lt2) {
      ...
      if (c.operator === "<" || c.operator === "<=") {
        ...
        lt2.operator === "<=" &&
        !satisfies$1(lt2.semver, String(c), options)
      ) {
        return false;
      }
    }
...
(function (module, exports) {
  ...
  var Buffer2 = moduleExports ? root2.Buffer : void 0,
    Symbol2 = root2.Symbol,
    Uint8Array2 = root2.Uint8Array,
  ...
  var DataView = getNative(root2, "DataView"),
    Map2 = getNative(root2, "Map"),
    Promise2 = getNative(root2, "Promise"),
    Set2 = getNative(root2, "Set"),
    WeakMap = getNative(root2, "WeakMap"),
...
class DownloadedUpdateHelper {
  ...
  async validateDownloadedPath(updateFile, updateInfo, fileInfo, logger) {
    if (
      ...
      if (
        ...
        (await (0, fs_extra_1$6.pathExists)(updateFile))
      ) {
        return updateFile;
      } else {
        return null;
    ...
    const cachedUpdateFile = await this.getValidCachedUpdateFile(
      fileInfo,
      logger,
    );
  ...
  async setDownloadedFile(
    ...
    if (isSaveCache) {
      await (0, fs_extra_1$6.outputJson)(
  ...
  async getValidCachedUpdateFile(fileInfo, logger) {
    ...
    const doesUpdateInfoFileExist = await (0, fs_extra_1$6.pathExists)(
      updateInfoFilePath,
    );
    ...
    const isCachedInfoFileNameValid =
      (cachedInfo === null || cachedInfo === void 0
        ? void 0
        : cachedInfo.fileName) !== null;
    ...
    const updateFile = path$9.join(
      this.cacheDirForPendingUpdate,
      cachedInfo.fileName,
    );
...
function getAppCacheDir() {
  ...
  if (process.platform === "win32") {
    result =
      process.env["LOCALAPPDATA"] || path$8.join(homedir, "AppData", "Local");
...
class ElectronAppAdapter {
  ...
  get appUpdateConfigPath() {
    return this.isPackaged
      ? path$7.join(process.resourcesPath, "app-update.yml")
      : path$7.join(this.app.getAppPath(), "dev-app-update.yml");
  }
  get userDataPath() {
    ...
    addRedirectHandlers(request, options, reject, redirectCount, handler) {
      request.on("redirect", (statusCode, method, redirectUrl) => {
        ...
        if (redirectCount > this.maxRedirects) {
          ...
          handler(
            builder_util_runtime_12.HttpExecutor.prepareRedirectUrlOptions(
              redirectUrl,
              options,
            ),
          );
...
class Provider {
  ...
  getBlockMapFiles(
    ...
    const newBlockMapUrl = (0, util_1$6.newUrlFromBase)(
      `${baseUrl.pathname}.blockmap`,
      baseUrl,
    );
    const oldBlockMapUrl = (0, util_1$6.newUrlFromBase)(
      `${baseUrl.pathname.replace(new RegExp(escapeRegExp$1(newVersion), "g"), oldVersion)}.blockmap`,
      oldBlockMapFileBaseUrl
        ? new url_1$5.URL(oldBlockMapFileBaseUrl)
        : baseUrl,
    );
  ...
  httpRequest(url, headers, cancellationToken) {
    return this.executor.request(
      this.createRequestOptions(url, headers),
      cancellationToken,
    );
  }
...
function findFile(files, extension, not) {
  var _a;
  if (files.length === 0) {
    throw (0, builder_util_runtime_1$f.newError)(
      "No files provided",
      "ERR_UPDATER_NO_FILES_PROVIDED",
    );
  }
  const filteredFiles = files.filter((it) =>
    it.url.pathname.toLowerCase().endsWith(`.${extension.toLowerCase()}`),
  );
  const result =
    (_a = filteredFiles.find((it) =>
  ...
  if (result) {
    ...
    return files.find(
      (fileInfo) =>
        !not.some((ext) =>
          fileInfo.url.pathname.toLowerCase().endsWith(`.${ext.toLowerCase()}`),
        ),
    );
  }
}
...
function getFileList(updateInfo) {
  ...
  if (updateInfo.path != null) {
    ...
    throw (0, builder_util_runtime_1$f.newError)(
      `No files provided: ${(0, builder_util_runtime_1$f.safeStringifyJson)(updateInfo)}`,
      "ERR_UPDATER_NO_FILES_PROVIDED",
    );
  }
}
function resolveFiles(updateInfo, baseUrl, pathTransformer = (p) => p) {
  ...
  const result = files.map((fileInfo) => {
    if (fileInfo.sha2 == null && fileInfo.sha512 == null) {
      throw (0, builder_util_runtime_1$f.newError)(
        `Update info doesn't contain nor sha256 neither sha512 checksum: ${(0, builder_util_runtime_1$f.safeStringifyJson)(fileInfo)}`,
  ...
  const packageInfo =
    packages == null ? null : packages[process.arch] || packages.ia32;
  if (packageInfo != null) {
    result[0].packageInfo = {
      ...packageInfo,
      path: (0, util_1$6.newUrlFromBase)(
        pathTransformer(packageInfo.path),
        baseUrl,
      ).href,
    };
  }
...
class GenericProvider extends Provider_1$b.Provider {
  ...
  async getLatestVersion() {
    ...
    const channelUrl = (0, util_1$5.newUrlFromBase)(
      channelFile,
      this.baseUrl,
    ...
    for (let attemptNumber = 0; ; attemptNumber++) {
      try {
        return (0, Provider_1$b.parseUpdateInfo)(
        ...
        if (
          e instanceof builder_util_runtime_1$e.HttpError &&
          e.statusCode === 404
        ) {
          throw (0, builder_util_runtime_1$e.newError)(
...
class BitbucketProvider extends Provider_1$a.Provider {
  constructor(configuration, updater, runtimeOptions) {
    ...
    this.baseUrl = (0, util_1$4.newBaseUrl)(
  ...
  async getLatestVersion() {
    ...
    const channelFile = (0, util_1$4.getChannelFilename)(
      this.getCustomChannelName(this.channel),
    );
    const channelUrl = (0, util_1$4.newUrlFromBase)(
      channelFile,
      this.baseUrl,
      this.updater.isAddNoCacheQuery,
    );
    try {
      ...
      return (0, Provider_1$a.parseUpdateInfo)(
        updateInfo,
        channelFile,
        channelUrl,
      );
    } catch (e) {
      throw (0, builder_util_runtime_1$d.newError)(
...
class BaseGitHubProvider extends Provider_1$9.Provider {
  constructor(options, defaultHost, runtimeOptions) {
    ...
    this.options = options;
    this.baseUrl = (0, util_1$3.newBaseUrl)(
      (0, builder_util_runtime_1$c.githubUrl)(options, defaultHost),
    );
    const apiHost =
      defaultHost === "github.com" ? "api.github.com" : defaultHost;
    this.baseApiUrl = (0, util_1$3.newBaseUrl)(
      (0, builder_util_runtime_1$c.githubUrl)(options, apiHost),
    );
  }
  computeGithubBasePath(result) {
    const host = this.options.host;
    return host && !["github.com", "api.github.com"].includes(host)
      ? `/api/v3${result}`
      : result;
  }
}
...
class GitHubProvider extends BaseGitHubProvider {
  ...
  async getLatestVersion() {
    ...
    const feedXml = await this.httpRequest(
      (0, util_1$3.newUrlFromBase)(`${this.basePath}.atom`, this.baseUrl),
      {
    ...
    try {
      if (this.updater.allowPrerelease) {
        const currentChannel =
          ((_a = this.updater) === null || _a === void 0
            ? void 0
            : _a.channel) ||
          ((_b = semver.prerelease(this.updater.currentVersion)) === null ||
          _b === void 0
            ? void 0
            : _b[0]) ||
          null;
        if (currentChannel === null) {
          ...
          for (const element of feed.getElements("entry")) {
            ...
            const hrefChannel =
              ((_c = semver.prerelease(hrefTag)) === null || _c === void 0
                ? void 0
                : _c[0]) || null;
        ...
        for (const element of feed.getElements("entry")) {
          const hrefMatch = hrefRegExp.exec(
            element.element("link").attribute("href"),
          );
          if (hrefMatch == null) {
            continue;
      ...
      throw (0, builder_util_runtime_1$c.newError)(
...
${feedXml}`,
    ...
    const fetchData = async (channelName) => {
      ...
      channelFileUrl = (0, util_1$3.newUrlFromBase)(
      ...
      try {
        ...
        if (
          e instanceof builder_util_runtime_1$c.HttpError &&
          e.statusCode === 404
        ) {
          throw (0, builder_util_runtime_1$c.newError)(
    ...
    try {
      ...
      if (
        ...
        ((_d = semver.prerelease(tag)) === null || _d === void 0
          ? void 0
          : _d[0])
      ) {
        channel = this.getCustomChannelName(
          String(
            (_e = semver.prerelease(tag)) === null || _e === void 0
              ? void 0
              : _e[0],
          ),
        );
    ...
    const result = (0, Provider_1$9.parseUpdateInfo)(
      rawData,
      channelFile,
      channelFileUrl,
    );
  ...
  resolveFiles(updateInfo) {
    return (0, Provider_1$9.resolveFiles)(updateInfo, this.baseUrl, (p) =>
...
function computeReleaseNotes(
  ...
  try {
    latestVersion = releaseVersionRegExp.exec(
      latestRelease.element("link").attribute("href"),
    )[1];
  ...
  for (const release of feed.getElements("entry")) {
    let versionRelease;
    try {
      const match = releaseVersionRegExp.exec(
...
class GitLabProvider extends Provider_1$8.Provider {
  ...
  async getLatestVersion() {
    ...
    const latestReleaseUrl = (0, util_1$2.newUrlFromBase)(
    ...
    let releaseResponse;
    try {
      releaseResponse = await this.httpRequest(
        latestReleaseUrl,
        header,
        cancellationToken,
      );
    } catch (e) {
      throw (0, builder_util_runtime_1$b.newError)(
        `Unable to find latest release on GitLab (${latestReleaseUrl}): ${e.stack || e.message}`,
        "ERR_UPDATER_LATEST_VERSION_NOT_FOUND",
      );
    }
    if (!releaseResponse) {
      throw (0, builder_util_runtime_1$b.newError)(
        "No published releases on GitLab",
        "ERR_UPDATER_NO_PUBLISHED_VERSIONS",
      );
    }
    let latestRelease;
    ...
    if (latestRelease.upcoming_release) {
      throw (0, builder_util_runtime_1$b.newError)(
        "Latest GitLab release is scheduled but not yet published",
        "ERR_UPDATER_LATEST_VERSION_NOT_FOUND",
      );
    }
    ...
    const fetchChannelData = async (channelName) => {
      ...
      const channelAsset = latestRelease.assets.links.find(
        (asset) => asset.name === channelFile,
      );
      if (!channelAsset) {
        throw (0, builder_util_runtime_1$b.newError)(
          `Cannot find ${channelFile} in the latest release assets`,
          "ERR_UPDATER_CHANNEL_FILE_NOT_FOUND",
        );
      }
      channelFileUrl = new url_1$3.URL(channelAsset.direct_asset_url);
      const authHeaders = this.setAuthHeaderForToken(
        this.options.token || null,
      );
      const headers = Object.keys(authHeaders).length ? authHeaders : void 0;
      try {
        const result2 = await this.httpRequest(
          channelFileUrl,
          headers,
          cancellationToken,
        );
        if (!result2) {
          throw (0, builder_util_runtime_1$b.newError)(
            `Empty response from ${channelFileUrl}`,
            "ERR_UPDATER_CHANNEL_FILE_NOT_FOUND",
          );
        }
        return result2;
        ...
        if (
          e instanceof builder_util_runtime_1$b.HttpError &&
          e.statusCode === 404
        ) {
          throw (0, builder_util_runtime_1$b.newError)(
            `Cannot find ${channelFile} in the latest release artifacts (${channelFileUrl}): ${e.stack || e.message}`,
    ...
    if (!rawData) {
      throw (0, builder_util_runtime_1$b.newError)(
        `Unable to parse channel data from ${channelFile}`,
        "ERR_UPDATER_INVALID_UPDATE_INFO",
      );
    }
    const result = (0, Provider_1$8.parseUpdateInfo)(
      rawData,
      channelFile,
      channelFileUrl,
    );
  ...
  findBlockMapInAssets(assets, filename) {
    const possibleBlockMapNames = [
      `${filename}.blockmap`,
      `${this.normalizeFilename(filename)}.blockmap`,
    ];
  ...
  async fetchReleaseInfoByVersion(version) {
    ...
    for (const releaseId of possibleReleaseIds) {
      const releaseUrl = (0, util_1$2.newUrlFromBase)(
      ...
      try {
        ...
        throw (0, builder_util_runtime_1$b.newError)(
          `Unable to find release ${releaseId} on GitLab (${releaseUrl}): ${e.stack || e.message}`,
          "ERR_UPDATER_RELEASE_NOT_FOUND",
        );
      }
    }
    throw (0, builder_util_runtime_1$b.newError)(
      `Unable to find release with version ${version} (tried: ${possibleReleaseIds.join(", ")}) on GitLab`,
      "ERR_UPDATER_RELEASE_NOT_FOUND",
    );
  }
  ...
  async getVersionInfoForBlockMap(version) {
    if (
      ...
      this.cachedLatestVersion.version === version
    ) {
  ...
  async findBlockMapUrlsFromAssets(oldVersion, newVersion, baseFilename) {
    ...
    if (newVersionAssets) {
      newBlockMapUrl = this.findBlockMapInAssets(
        newVersionAssets,
        baseFilename,
      );
    }
    ...
    if (oldVersionAssets) {
      const oldFilename = baseFilename.replace(
        new RegExp(escapeRegExp(newVersion), "g"),
        oldVersion,
      );
  ...
  async getBlockMapFiles(
    ...
    if (this.options.uploadTarget === "project_upload") {
      ...
      const [oldBlockMapUrl, newBlockMapUrl] =
        await this.findBlockMapUrlsFromAssets(
          oldVersion,
          newVersion,
          baseFilename,
        );
      if (!newBlockMapUrl) {
        throw (0, builder_util_runtime_1$b.newError)(
          `Cannot find blockmap file for ${newVersion} in GitLab assets`,
          "ERR_UPDATER_BLOCKMAP_FILE_NOT_FOUND",
        );
      }
      if (!oldBlockMapUrl) {
        throw (0, builder_util_runtime_1$b.newError)(
          `Cannot find blockmap file for ${oldVersion} in GitLab assets`,
          "ERR_UPDATER_BLOCKMAP_FILE_NOT_FOUND",
        );
      }
      return [oldBlockMapUrl, newBlockMapUrl];
    } else {
      return super.getBlockMapFiles(
        baseUrl,
        oldVersion,
        newVersion,
        oldBlockMapFileBaseUrl,
  ...
  resolveFiles(updateInfo) {
    return (0, Provider_1$8.getFileList)(updateInfo).map((fileInfo) => {
      ...
      const matchingAssetName = possibleNames.find((name) =>
        updateInfo.assets.has(name),
      );
      const assetUrl = matchingAssetName
        ? updateInfo.assets.get(matchingAssetName)
        : void 0;
      if (!assetUrl) {
        throw (0, builder_util_runtime_1$b.newError)(
...
class KeygenProvider extends Provider_1$7.Provider {
  constructor(configuration, updater, runtimeOptions) {
    ...
    this.baseUrl = (0, util_1$1.newBaseUrl)(
      `https://${host}/v1/accounts/${this.configuration.account}/artifacts?product=${this.configuration.product}`,
    );
  }
  get channel() {
  ...
  async getLatestVersion() {
    ...
    const channelFile = (0, util_1$1.getChannelFilename)(
      this.getCustomChannelName(this.channel),
    );
    const channelUrl = (0, util_1$1.newUrlFromBase)(
      channelFile,
      this.baseUrl,
      this.updater.isAddNoCacheQuery,
    );
    try {
      ...
      return (0, Provider_1$7.parseUpdateInfo)(
        updateInfo,
        channelFile,
        channelUrl,
      );
    } catch (e) {
      throw (0, builder_util_runtime_1$a.newError)(
        `Unable to find latest version on ${this.toString()}, please ensure release exists: ${e.stack || e.message}`,
        "ERR_UPDATER_LATEST_VERSION_NOT_FOUND",
      );
    }
...
class PrivateGitHubProvider extends GitHubProvider_1$1.BaseGitHubProvider {
  ...
  async getLatestVersion() {
    ...
    const channelFile = (0, util_1.getChannelFilename)(
      this.getDefaultChannelName(),
    );
    const releaseInfo = await this.getLatestVersionInfo(cancellationToken);
    const asset = releaseInfo.assets.find((it) => it.name === channelFile);
    if (asset == null) {
      throw (0, builder_util_runtime_1$9.newError)(
    ...
    try {
      ...
      if (
        e instanceof builder_util_runtime_1$9.HttpError &&
        e.statusCode === 404
      ) {
        throw (0, builder_util_runtime_1$9.newError)(
...
providerFactory.isUrlProbablySupportMultiRangeRequests =
...
function createClient(data, updater, runtimeOptions) {
  if (typeof data === "string") {
    throw (0, builder_util_runtime_1$8.newError)(
  ...
  switch (provider) {
    case "github": {
      ...
      const token =
        (githubOptions.private
          ? process.env["GH_TOKEN"] || process.env["GITHUB_TOKEN"]
          : null) || githubOptions.token;
      if (token == null) {
        return new GitHubProvider_1.GitHubProvider(
          githubOptions,
          updater,
          runtimeOptions,
        );
      } else {
        return new PrivateGitHubProvider_1.PrivateGitHubProvider(
          githubOptions,
          updater,
          token,
          runtimeOptions,
    ...
    case "bitbucket":
      return new BitbucketProvider_1.BitbucketProvider(
        data,
        updater,
        runtimeOptions,
      );
    ...
    case "generic": {
      ...
      return new GenericProvider_1$1.GenericProvider(options, updater, {
        ...
        isUseMultipleRangeRequest:
          options.useMultipleRangeRequest !== false &&
          isUrlProbablySupportMultiRangeRequests(options.url),
      });
    }
    case "custom": {
      const options = data;
...
function computeOperations(oldBlockMap, newBlockMap, logger) {
  ...
  const { checksumToOffset: checksumToOldOffset, checksumToOldSize } =
  ...
  for (
    ...
    if (oldOffset != null && checksumToOldSize.get(checksum) !== blockSize) {
      logger.warn(
        `Checksum ("${checksum}") matches, but size differs (old: ${checksumToOldSize.get(checksum)}, new: ${blockSize})`,
      );
      oldOffset = void 0;
    }
    if (oldOffset === void 0) {
      ...
      if (
        lastOperation != null &&
        lastOperation.kind === OperationKind$1.DOWNLOAD &&
      ...
      if (
        lastOperation != null &&
        lastOperation.kind === OperationKind$1.COPY &&
  ...
  if (changedBlockCount > 0) {
    logger.info(
      `File${blockMapFile.name === "file" ? "" : " " + blockMapFile.name} has ${changedBlockCount} changed blocks`,
    );
  }
  return operations;
}
const isValidateOperationRange =
...
class DataSplitter extends stream_1$1.Writable {
  ...
  async handleData(chunk) {
    ...
    if (this.ignoreByteCount !== 0 && this.remainingPartDataCount !== 0) {
      throw (0, builder_util_runtime_1$7.newError)(
        "Internal error",
  ...
  onPartEnd() {
    ...
    if (this.actualPartLength !== expectedLength) {
      throw (0, builder_util_runtime_1$7.newError)(
        `Expected length: ${expectedLength} differs from actual: ${this.actualPartLength}`,
  ...
  processPartData(data, start, end) {
    ...
    if (
      out2.write(
        start === 0 && data.length === end ? data : data.slice(start, end),
      )
    ) {
...
multipleRangeDownloader.executeTasksUsingMultipleRangeRequests =
...
function executeTasksUsingMultipleRangeRequests(
...
function doExecuteTasks(
  ...
  if (partCount <= 1) {
    const w = (index) => {
      ...
      if (task.kind === downloadPlanBuilder_1$1.OperationKind.COPY) {
        ...
        const request2 = differentialDownloader.httpExecutor.createRequest(
          requestOptions2,
          (response) => {
            response.on("error", reject);
        ...
        differentialDownloader.httpExecutor.addErrorAndTimeoutHandlers(
          request2,
          reject,
        );
        request2.end();
  ...
  const request = differentialDownloader.httpExecutor.createRequest(
    requestOptions,
    (response) => {
      if (!checkIsRangesSupported(response, reject)) {
        return;
      }
      const contentType = (0, builder_util_runtime_1$6.safeGetHeader)(
        response,
        "content-type",
      );
      const m =
        /^multipart\/.+?\s*;\s*boundary=(?:"([^"]+)"|([^\s";]+))\s*$/i.exec(
          contentType,
        );
      if (m == null) {
        reject(
          new Error(
            `Content-Type "multipart/byteranges" is expected, but got "${contentType}"`,
          ),
        );
        return;
      }
      const dicer = new DataSplitter_1$1.DataSplitter(
        out2,
        options,
        partIndexToTaskIndex,
        m[1] || m[2],
...
function checkIsRangesSupported(response, reject) {
  ...
  if (response.statusCode !== 206) {
    const acceptRanges = (0, builder_util_runtime_1$6.safeGetHeader)(
      response,
      "accept-ranges",
    );
    if (acceptRanges == null || acceptRanges === "none") {
      reject(
        new Error(
          `Server doesn't support Accept-Ranges (response code ${response.statusCode})`,
        ),
      );
      return false;
    }
...
Object.defineProperty(
  ProgressDifferentialDownloadCallbackTransform$1,
  "__esModule",
  { value: true },
);
ProgressDifferentialDownloadCallbackTransform$1.ProgressDifferentialDownloadCallbackTransform =
  void 0;
...
const ProgressDifferentialDownloadCallbackTransform_1 =
  ProgressDifferentialDownloadCallbackTransform$1;
class DifferentialDownloader {
  ...
  createRequestOptions() {
    ...
    (0, builder_util_runtime_1$5.configureRequestUrl)(
      this.options.newUrl,
      result,
    );
  ...
  doDownload(oldBlockMap, newBlockMap) {
    ...
    const operations = (0, downloadPlanBuilder_1.computeOperations)(
      oldBlockMap,
      newBlockMap,
      logger,
    );
    ...
    logger.info(
      `Full: ${formatBytes(newSize)}, To download: ${formatBytes(downloadSize)} (${Math.round(downloadSize / (newSize / 100))}%)`,
    );
  ...
  async doDownloadFile(tasks, fdList) {
    ...
    const fileOut = (0, fs_1$2.createWriteStream)(this.options.newFile, {
      fd: newFileFd,
    });
    await new Promise((resolve, reject) => {
      const streams = [];
      ...
      if (!this.options.isUseMultipleRangeRequest && this.options.onProgress) {
        ...
        downloadInfoTransform =
          new ProgressDifferentialDownloadCallbackTransform_1.ProgressDifferentialDownloadCallbackTransform(
      ...
      const digestTransform = new builder_util_runtime_1$5.DigestTransform(
      ...
      if (this.options.isUseMultipleRangeRequest) {
        w = (0,
        multipleRangeDownloader_1.executeTasksUsingMultipleRangeRequests)(
          this,
          tasks,
          firstStream,
          oldFileFd,
      ...
      w = (index) => {
        ...
        (_b =
          ...
          null || _b === void 0
          ? void 0
          : _b.call(_a, `download range: ${range2}`);
        if (downloadInfoTransform) {
  ...
  request(requestOptions, dataHandler) {
    return new Promise((resolve, reject) => {
      const request = this.httpExecutor.createRequest(
        ...
        (response) => {
          if (
            !(0, multipleRangeDownloader_1.checkIsRangesSupported)(
              response,
              reject,
            )
          ) {
...
Object.defineProperty(GenericDifferentialDownloader$1, "__esModule", {
  value: true,
});
...
class GenericDifferentialDownloader
  extends DifferentialDownloader_1$1.DifferentialDownloader
{
  download(oldBlockMap, newBlockMap) {
    return this.doDownload(oldBlockMap, newBlockMap);
  }
}
GenericDifferentialDownloader$1.GenericDifferentialDownloader =
  GenericDifferentialDownloader;
var types = {};
(function (exports) {
  Object.defineProperty(exports, "__esModule", { value: true });
  exports.UpdaterSignal =
    exports.UPDATE_DOWNLOADED =
...
class AppUpdater extends events_1.EventEmitter {
  ...
  set channel(value) {
    if (this._channel != null) {
      if (typeof value !== "string") {
        throw (0, builder_util_runtime_1$4.newError)(
  ...
  constructor(options, app2) {
    ...
    this._isUpdateSupported = (updateInfo) =>
      this.checkIfUpdateSupported(updateInfo);
    this._isUserWithinRollout = (updateInfo) => this.isStagingMatch(updateInfo);
    this.clientPromise = null;
    this.stagingUserIdPromise = new lazy_val_1.Lazy(() =>
    ...
    if (app2 == null) {
      ...
      this.httpExecutor = new electronHttpExecutor_1.ElectronHttpExecutor(
        (authInfo, callback) => this.emit("login", authInfo, callback),
      );
    } else {
      this.app = app2;
  ...
  setFeedURL(options) {
    ...
    let provider;
    if (typeof options === "string") {
      provider = new GenericProvider_1.GenericProvider(
        { provider: "generic", url: options },
        this,
        {
          ...runtimeOptions,
          isUseMultipleRangeRequest: (0,
          providerFactory_1.isUrlProbablySupportMultiRangeRequests)(options),
        },
      );
    } else {
      provider = (0, providerFactory_1.createClient)(
        options,
        this,
        runtimeOptions,
      );
  ...
  async isStagingMatch(updateInfo) {
    ...
    const val =
      builder_util_runtime_1$4.UUID.parse(stagingUserId).readUInt32BE(12);
  ...
  async isUpdateAvailable(updateInfo) {
    ...
    const isUserWithinRollout = await Promise.resolve(
      this.isUserWithinRollout(updateInfo),
    );
    if (!isUserWithinRollout) {
      return false;
    }
    const isLatestVersionNewer = (0, semver_1.gt)(
      latestVersion,
      currentVersion,
    );
    const isLatestVersionOlder = (0, semver_1.lt)(
      latestVersion,
      currentVersion,
    );
  ...
  checkIfUpdateSupported(updateInfo) {
    const minimumSystemVersion =
      updateInfo === null || updateInfo === void 0
        ? void 0
    ...
    if (minimumSystemVersion) {
      try {
        if ((0, semver_1.lt)(currentOSVersion, minimumSystemVersion)) {
          this._logger.info(
            `Current OS version ${currentOSVersion} is less than the minimum OS version required ${minimumSystemVersion} for version ${currentOSVersion}`,
          );
          return false;
        }
      } catch (e) {
        this._logger.warn(
          `Failed to compare current OS version(${currentOSVersion}) with minimum OS version(${minimumSystemVersion}): ${(e.message || e).toString()}`,
        );
      }
    }
    return true;
  ...
  async getUpdateInfoAndProvider() {
    ...
    if (this.clientPromise == null) {
      this.clientPromise = this.configOnDisk.value.then((it) =>
        (0, providerFactory_1.createClient)(
          it,
          this,
  ...
  onUpdateAvailable(updateInfo) {
    this._logger.info(
      `Found version ${updateInfo.version} (url: ${(0,
      builder_util_runtime_1$4.asArray)(updateInfo.files)
        .map((it) => it.url)
        .join(", ")})`,
    );
   ...
   */
  downloadUpdate(
    cancellationToken = new builder_util_runtime_1$4.CancellationToken(),
  ) {
    ...
    this._logger.info(
      `Downloading update from ${(0, builder_util_runtime_1$4.asArray)(
        updateInfoAndProvider.info.files,
      )
        .map((it) => it.url)
        .join(", ")}`,
    );
  ...
  async loadUpdateConfig() {
    ...
    return (0, js_yaml_1.load)(
      await (0, fs_extra_1$4.readFile)(this._appUpdateConfigPath, "utf-8"),
    );
  }
  ...
  async getOrCreateStagingUserId() {
    ...
    try {
      ...
      if (builder_util_runtime_1$4.UUID.check(id2)) {
        ...
        this._logger.warn(
          `Staging user id file exists, but content was invalid: ${id2}`,
        );
      }
    } catch (e) {
      if (e.code !== "ENOENT") {
    ...
    const id = builder_util_runtime_1$4.UUID.v5(
      (0, crypto_1$1.randomBytes)(4096),
      builder_util_runtime_1$4.UUID.OID,
    );
  ...
  async executeDownload(taskOptions) {
    ...
    if (this.listenerCount(types_1$5.DOWNLOAD_PROGRESS) > 0) {
      downloadOptions.onProgress = (it) =>
        this.emit(types_1$5.DOWNLOAD_PROGRESS, it);
    }
    const updateInfo =
    ...
    function getCacheUpdateFileName() {
      ...
      if (
        urlPath
          .toLowerCase()
          .endsWith(`.${taskOptions.fileExtension.toLowerCase()}`)
      ) {
    ...
    const packageFile =
      packageInfo == null
        ...
        : path$5.join(
            cacheDir,
            `package-${version}${path$5.extname(packageInfo.path) || ".7z"}`,
          );
    const done = async (isSaveCache) => {
      ...
      if (await (0, fs_extra_1$4.pathExists)(currentBlockMapFile)) {
        await (0, fs_extra_1$4.copyFile)(
          currentBlockMapFile,
          path$5.join(downloadedUpdateHelper.cacheDir, "current.blockmap"),
        );
      }
      return packageFile == null ? [updateFile] : [updateFile, packageFile];
    };
    const log = this._logger;
    const cachedUpdateFile =
      await downloadedUpdateHelper.validateDownloadedPath(
        updateFile,
        updateInfo,
        fileInfo,
        log,
    ...
    const tempUpdateFile = await (0,
    DownloadedUpdateHelper_1.createTempUpdateFile)(
      `temp-${updateFileName}`,
      cacheDir,
      log,
    );
    try {
      ...
      await (0, builder_util_runtime_1$4.retry)(
        () => (0, fs_extra_1$4.rename)(tempUpdateFile, updateFile),
        {
          retries: 60,
          interval: 500,
          shouldRetry: (error2) => {
            ...
            log.warn(
              `Cannot rename temp file to final file: ${error2.message || error2.stack}`,
            );
            return false;
          },
        },
  ...
  async differentialDownloadInstaller(
    fileInfo,
    ...
    try {
      ...
      const blockmapFileUrls = await provider2.getBlockMapFiles(
        fileInfo.url,
        this.app.version,
      ...
      this._logger.info(
        `Download block maps (old: "${blockmapFileUrls[0]}", new: ${blockmapFileUrls[1]})`,
      );
      ...
      const saveBlockMapToCacheDir = async (blockMapData, cacheDir) => {
        ...
        await (0, fs_extra_1$4.outputFile)(
          blockMapFile,
          (0, zlib_1$1.gzipSync)(JSON.stringify(blockMapData)),
        );
      };
      const getBlockMapFromCacheDir = async (cacheDir) => {
        ...
        try {
          if (await (0, fs_extra_1$4.pathExists)(blockMapFile)) {
            return JSON.parse(
              (0, zlib_1$1.gunzipSync)(
                await (0, fs_extra_1$4.readFile)(blockMapFile),
              ).toString(),
            );
          }
        } catch (e) {
          this._logger.warn(
            `Cannot parse blockmap "${blockMapFile}", error: ${e}`,
          );
        }
        return null;
      };
      const newBlockMapData = await downloadBlockMap(blockmapFileUrls[1]);
      await saveBlockMapToCacheDir(
        newBlockMapData,
        this.downloadedUpdateHelper.cacheDirForPendingUpdate,
      );
      let oldBlockMapData = await getBlockMapFromCacheDir(
        this.downloadedUpdateHelper.cacheDir,
      );
      if (oldBlockMapData == null) {
        oldBlockMapData = await downloadBlockMap(blockmapFileUrls[0]);
      }
      await new GenericDifferentialDownloader_1.GenericDifferentialDownloader(
        fileInfo.info,
        this.httpExecutor,
        downloadOptions,
      ).download(oldBlockMapData, newBlockMapData);
      return false;
    } catch (e) {
      this._logger.error(
        `Cannot download differentially, fallback to full download: ${e.stack || e}`,
      );
...
class BaseUpdater extends AppUpdater_1$1.AppUpdater {
  ...
  quitAndInstall(isSilent = false, isForceRunAfter = false) {
    ...
    const isInstalled = this.install(
      isSilent,
      isSilent ? isForceRunAfter : this.autoRunAppAfterInstall,
    );
    if (isInstalled) {
      setImmediate(() => {
  ...
  sanitizeEnvPath(envPath) {
    return envPath
      .split(path$4.delimiter)
      .filter((dir) => path$4.isAbsolute(dir))
      .join(path$4.delimiter);
  }
  spawnSyncLog(cmd, args = [], env = {}) {
    ...
    const response = (0, child_process_1$3.spawnSync)(cmd, args, {
      env: {
        ...mergedEnv,
        PATH: this.sanitizeEnvPath(
          (_a = mergedEnv.PATH) !== null && _a !== void 0 ? _a : "",
        ),
      },
      encoding: "utf-8",
      shell: true,
...
Object.defineProperty(
  FileWithEmbeddedBlockMapDifferentialDownloader$1,
  "__esModule",
  { value: true },
);
FileWithEmbeddedBlockMapDifferentialDownloader$1.FileWithEmbeddedBlockMapDifferentialDownloader =
  void 0;
const fs_extra_1$3 = lib;
const DifferentialDownloader_1 = DifferentialDownloader$1;
const zlib_1 = require$$14;
class FileWithEmbeddedBlockMapDifferentialDownloader
  extends DifferentialDownloader_1.DifferentialDownloader
{
  async download() {
    ...
    const newBlockMap = readBlockMap(
      this.fileMetadataBuffer.slice(0, this.fileMetadataBuffer.length - 4),
    );
    await this.doDownload(
      await readEmbeddedBlockMapData(this.options.oldFile),
      newBlockMap,
    );
  }
}
...
const FileWithEmbeddedBlockMapDifferentialDownloader_1$1 =
  FileWithEmbeddedBlockMapDifferentialDownloader$1;
...
class AppImageUpdater extends BaseUpdater_1$2.BaseUpdater {
  ...
  doDownloadUpdate(downloadUpdateOptions) {
    ...
    const fileInfo = (0, Provider_1$5.findFile)(
    ...
    return this.executeDownload({
      ...
      task: async (updateFile, downloadOptions) => {
        ...
        if (oldFile == null) {
          throw (0, builder_util_runtime_1$3.newError)(
        ...
        if (
          ...
          (await this.downloadDifferential(
            fileInfo,
            oldFile,
            updateFile,
            provider,
  ...
  async downloadDifferential(
    ...
    try {
      ...
      if (this.listenerCount(types_1$4.DOWNLOAD_PROGRESS) > 0) {
        downloadOptions.onProgress = (it) =>
          this.emit(types_1$4.DOWNLOAD_PROGRESS, it);
      }
      await new FileWithEmbeddedBlockMapDifferentialDownloader_1$1.FileWithEmbeddedBlockMapDifferentialDownloader(
        fileInfo.info,
        this.httpExecutor,
        downloadOptions,
      ).download();
  ...
  doInstall(options) {
    ...
    if (!path$3.isAbsolute(appImageFile) || appImageFile.includes("\0")) {
      throw (0, builder_util_runtime_1$3.newError)(
        `APPIMAGE env is not a valid absolute path: "${appImageFile}"`,
        "ERR_UPDATER_OLD_FILE_NOT_FOUND",
      );
    }
    ...
    if (
      path$3.basename(installerPath) === existingBaseName ||
      !/\d+\.\d+\.\d+/.test(existingBaseName)
    ) {
      destination = appImageFile;
    } else {
      destination = path$3.join(
        path$3.dirname(appImageFile),
        path$3.basename(installerPath),
      );
    }
...
class LinuxUpdater extends BaseUpdater_1$1.BaseUpdater {
  ...
  isRunningAsRoot() {
    var _a;
    return (
      ((_a = process.getuid) === null || _a === void 0
        ? void 0
        : _a.call(process)) === 0
    );
  }
  ...
  runCommandWithSudoIfNeeded(commandWithArgs) {
    ...
    this._logger.info(
      `Running as non-root user, using sudo to install: ${sudo}`,
    );
    let wrapper = `"`;
  ...
  detectPackageManager(pms) {
    ...
    const pmOverride =
      (_a = process.env.ELECTRON_BUILDER_LINUX_PACKAGE_MANAGER) === null ||
      _a === void 0
        ? void 0
        : _a.trim();
    if (pmOverride) {
      if (!SAFE_PM_REGEX.test(pmOverride)) {
        this._logger.warn(
          `ELECTRON_BUILDER_LINUX_PACKAGE_MANAGER "${pmOverride}" contains unsafe characters. Ignoring override.`,
        );
      } else {
    ...
    const searchList = pmOverride
      ? `ELECTRON_BUILDER_LINUX_PACKAGE_MANAGER override "${pmOverride}", `
      : "";
    const defaultPM = pms[0];
    this._logger.warn(
      `No package manager found in the list: ${searchList}${pms.join(", ")}. Utilizing default: ${defaultPM}`,
    );
    return defaultPM;
  }
}
...
class DebUpdater extends LinuxUpdater_1$2.LinuxUpdater {
  ...
  doDownloadUpdate(downloadUpdateOptions) {
    const provider = downloadUpdateOptions.updateInfoAndProvider.provider;
    const fileInfo = (0, Provider_1$4.findFile)(
      provider.resolveFiles(downloadUpdateOptions.updateInfoAndProvider.info),
      "deb",
      ["AppImage", "rpm", "pacman"],
    );
    return this.executeDownload({
      ...
      task: async (updateFile, downloadOptions) => {
        if (this.listenerCount(types_1$3.DOWNLOAD_PROGRESS) > 0) {
          downloadOptions.onProgress = (it) =>
            this.emit(types_1$3.DOWNLOAD_PROGRESS, it);
        }
        await this.httpExecutor.download(
          fileInfo.url,
          updateFile,
  ...
  static installWithCommandRunner(
    ...
    if (packageManager === "dpkg") {
      try {
        ...
        logger.warn(
          (_a = error2.message) !== null && _a !== void 0 ? _a : error2,
        );
        logger.warn(
...
class PacmanUpdater extends LinuxUpdater_1$1.LinuxUpdater {
  ...
  doDownloadUpdate(downloadUpdateOptions) {
    const provider = downloadUpdateOptions.updateInfoAndProvider.provider;
    const fileInfo = (0, Provider_1$3.findFile)(
      provider.resolveFiles(downloadUpdateOptions.updateInfoAndProvider.info),
      "pacman",
      ["AppImage", "deb", "rpm"],
    );
    return this.executeDownload({
      ...
      task: async (updateFile, downloadOptions) => {
        if (this.listenerCount(types_1$2.DOWNLOAD_PROGRESS) > 0) {
          downloadOptions.onProgress = (it) =>
            this.emit(types_1$2.DOWNLOAD_PROGRESS, it);
        }
        await this.httpExecutor.download(
          fileInfo.url,
          updateFile,
...
class RpmUpdater extends LinuxUpdater_1.LinuxUpdater {
  ...
  doDownloadUpdate(downloadUpdateOptions) {
    const provider = downloadUpdateOptions.updateInfoAndProvider.provider;
    const fileInfo = (0, Provider_1$2.findFile)(
      provider.resolveFiles(downloadUpdateOptions.updateInfoAndProvider.info),
      "rpm",
      ["AppImage", "deb", "pacman"],
    );
    return this.executeDownload({
      ...
      task: async (updateFile, downloadOptions) => {
        if (this.listenerCount(types_1$1.DOWNLOAD_PROGRESS) > 0) {
          downloadOptions.onProgress = (it) =>
            this.emit(types_1$1.DOWNLOAD_PROGRESS, it);
        }
        await this.httpExecutor.download(
          fileInfo.url,
          updateFile,
...
class MacUpdater extends AppUpdater_1.AppUpdater {
  ...
  static filterFilesForArch(files, isArm64Mac) {
    const isArm64File = (file2) => {
      var _a;
      return (
        file2.url.pathname.includes("arm64") ||
        ((_a = file2.info.url) === null || _a === void 0
          ? void 0
          : _a.includes("arm64"))
      );
    };
  ...
  async doDownloadUpdate(downloadUpdateOptions) {
    let files =
      downloadUpdateOptions.updateInfoAndProvider.provider.resolveFiles(
    ...
    try {
      ...
      const result = (0, child_process_1$1.execFileSync)(
        "sysctl",
        [sysctlRosettaInfoKey],
        { encoding: "utf8" },
      );
      isRosetta = result.includes(`${sysctlRosettaInfoKey}: 1`);
      log.info(
        `Checked for macOS Rosetta environment (isRosetta=${isRosetta})`,
      );
    } catch (e) {
      log.warn(
        `sysctl shell command to check for macOS Rosetta environment failed: ${e}`,
      );
    }
    let isArm64Mac = false;
    try {
      this.debug("Checking for arm64 in uname");
      const result = (0, child_process_1$1.execFileSync)("uname", ["-a"], {
        encoding: "utf8",
      });
    ...
    const zipFileInfo = (0, Provider_1$1.findFile)(files, "zip", [
      "pkg",
      "dmg",
    ]);
    if (zipFileInfo == null) {
      throw (0, builder_util_runtime_1$2.newError)(
        `ZIP file not provided: ${(0, builder_util_runtime_1$2.safeStringifyJson)(files)}`,
    ...
    return this.executeDownload({
      ...
      task: async (destinationFile, downloadOptions) => {
        const cachedUpdateFilePath = path$2.join(
          this.downloadedUpdateHelper.cacheDir,
          CURRENT_MAC_APP_ZIP_FILE_NAME,
        );
        ...
        if (canDifferentialDownload()) {
          differentialDownloadFailed = await this.differentialDownloadInstaller(
            zipFileInfo,
            downloadUpdateOptions,
            destinationFile,
            provider,
        ...
        if (!downloadUpdateOptions.disableDifferentialDownload) {
          try {
            ...
            await (0, fs_extra_1$1.copyFile)(
              event.downloadedFile,
              cachedUpdateFilePath,
            );
          } catch (error2) {
            this._logger.warn(
              `Unable to copy file for caching for future differential downloads: ${error2.message}`,
            );
          }
        }
  ...
  async updateDownloaded(zipFileInfo, event) {
    ...
    const updateFileSize =
      (_a = zipFileInfo.info.size) !== null && _a !== void 0
        ? _a
        : (await (0, fs_extra_1$1.stat)(downloadedFile)).size;
    const log = this._logger;
    ...
    this.debug(
      `Proxy server for native Squirrel.Mac is created (${logContext})`,
    );
    this.server.on("close", () => {
      log.info(
        `Proxy server for native Squirrel.Mac is closed (${logContext})`,
      );
    });
    ...
    return await new Promise((resolve, reject) => {
      const pass = (0, crypto_1.randomBytes)(64)
        .toString("base64")
        .replace(/\//g, "_")
        .replace(/\+/g, "-");
      ...
      this.server.on("request", (request, response) => {
        ...
        if (requestUrl === "/") {
          ...
          const data = Buffer.from(
            `{ "url": "${getServerUrl(this.server)}${fileUrl}" }`,
          );
      ...
      this.debug(
        `Proxy server for native Squirrel.Mac is starting to listen (${logContext})`,
      );
      this.server.listen(0, "127.0.0.1", () => {
        this.debug(
          `Proxy server for native Squirrel.Mac is listening (address=${getServerUrl(this.server)}, ${logContext})`,
        );
...
Object.defineProperty(windowsExecutableCodeSignatureVerifier, "__esModule", {
  value: true,
});
        ...
        try {
          ...
          if (data.Status === 0) {
            try {
              ...
              const normalizedTempUpdateFile = path$1.normalize(
                unescapedTempUpdateFile,
              );
              logger.info(
                `LiteralPath: ${normlaizedUpdateFilePath}. Update Path: ${normalizedTempUpdateFile}`,
              );
              if (normlaizedUpdateFilePath !== normalizedTempUpdateFile) {
                handleError(
                  logger,
                  new Error(
                    `LiteralPath of ${normlaizedUpdateFilePath} is different than ${normalizedTempUpdateFile}`,
                  ),
                  stderr,
                  reject,
                );
              ...
              logger.warn(
                `Unable to verify LiteralPath of update asset due to missing data.Path. Skipping this step of validation. Message: ${(_a = error3.message) !== null && _a !== void 0 ? _a : error3.stack}`,
              );
            }
            const subject = (0, builder_util_runtime_1$1.parseDn)(
          ...
          const result =
            ...
            JSON.stringify(
              data,
              (name, value) => (name === "RawData" ? void 0 : value),
              2,
            );
          logger.warn(
...
function handleError(logger, error2, stderr, reject) {
  if (isOldWin6()) {
    logger.warn(
      `Cannot execute Get-AuthenticodeSignature: ${error2 || stderr}. Ignoring signature validation due to unsupported powershell version. Please upgrade to powershell 3 or higher.`,
    );
    return;
  }
  try {
    (0, child_process_1.execFileSync)(
      ...preparePowerShellExec("ConvertTo-Json test", 10 * 1e3),
    );
  } catch (testError) {
    logger.warn(
...
const FileWithEmbeddedBlockMapDifferentialDownloader_1 =
  FileWithEmbeddedBlockMapDifferentialDownloader$1;
const types_1 = types;
const Provider_1 = Provider$1;
const fs_extra_1 = lib;
const windowsExecutableCodeSignatureVerifier_1 =
  windowsExecutableCodeSignatureVerifier;
const url_1 = require$$2$1;
class NsisUpdater extends BaseUpdater_1.BaseUpdater {
  constructor(options, app2) {
    super(options, app2);
    this._verifyUpdateCodeSignature = (
      publisherNames,
      unescapedTempUpdateFile,
    ) =>
      (0, windowsExecutableCodeSignatureVerifier_1.verifySignature)(
        publisherNames,
        unescapedTempUpdateFile,
        this._logger,
      );
  ...
  doDownloadUpdate(downloadUpdateOptions) {
    const provider = downloadUpdateOptions.updateInfoAndProvider.provider;
    const fileInfo = (0, Provider_1.findFile)(
      provider.resolveFiles(downloadUpdateOptions.updateInfoAndProvider.info),
      "exe",
    );
    return this.executeDownload({
      ...
      task: async (
        ...
        if (isWebInstaller && downloadUpdateOptions.disableWebInstaller) {
          throw (0, builder_util_runtime_1.newError)(
        ...
        if (
          ...
          (await this.differentialDownloadInstaller(
            ...
            destinationFile,
            provider,
            builder_util_runtime_1.CURRENT_APP_INSTALLER_FILE_NAME,
          ))
        ) {
          await this.httpExecutor.download(
            fileInfo.url,
        ...
        const signatureVerificationStatus =
          await this.verifySignature(destinationFile);
        ...
        if (isWebInstaller) {
          if (
            ...
            try {
              await this.httpExecutor.download(
                new url_1.URL(packageInfo.path),
                packageFile,
                {
                  headers: downloadUpdateOptions.requestHeaders,
                  cancellationToken: downloadUpdateOptions.cancellationToken,
  ...
  async differentialDownloadWebPackage(
    downloadUpdateOptions,
    packageInfo,
    packagePath,
    provider,
    ...
    try {
      ...
      if (this.listenerCount(types_1.DOWNLOAD_PROGRESS) > 0) {
        downloadOptions.onProgress = (it) =>
          this.emit(types_1.DOWNLOAD_PROGRESS, it);
      }
      await new FileWithEmbeddedBlockMapDifferentialDownloader_1.FileWithEmbeddedBlockMapDifferentialDownloader(
        packageInfo,
        this.httpExecutor,
        downloadOptions,
      ).download();
...
(function (exports) {
  ...
  exports.NsisUpdater =
    exports.MacUpdater =
    exports.RpmUpdater =
    exports.PacmanUpdater =
    exports.DebUpdater =
    exports.AppImageUpdater =
    exports.Provider =
    exports.NoOpLogger =
    exports.AppUpdater =
    exports.BaseUpdater =
  ...
  function doLoadAutoUpdater() {
    if (process.platform === "win32") {
      ...
      try {
        ...
        const fileType = (0, fs_extra_12.readFileSync)(identity)
          .toString()
          .trim();
        switch (fileType) {
          case "deb":
...
process.env.VITE_PUBLIC = VITE_DEV_SERVER_URL
  ? path$n.join(process.env.APP_ROOT, "public")
  : RENDERER_DIST;
let win;
function createWindow() {
  win = new BrowserWindow({
    frame: false,
    icon: path$n.join(process.env.VITE_PUBLIC, "icon.svg"),
    height: 500,
    minHeight: 500,
    width: 700,
    minWidth: 700,
...
function getUnique5DigitCode() {
  const timestampStr = String(Date.now() / 1e3);
...
function genRecLabelZpl(uniqueCode, counter, manualCode, mode) {
...
function genPositionLabelZpl(position, hasQR, fontSize) {
  if (hasQR) {
